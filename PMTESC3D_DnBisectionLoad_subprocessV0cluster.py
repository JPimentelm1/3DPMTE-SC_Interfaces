    ...
    for job_obj in Joblist:
        ...
        if k > 1:
            ...
            if eps0<bisectol and NeL2damagek>0 and stepdata[-1][-1]<0:
                eps0file = open('eps0_value.txt', 'w')
            ...
            chdir(work_dir)
            # List of file extensions to delete
            extensions_to_delete = ['*.inp','*.dat', '*.sim', '*.com', '*.jnl', '*.mdl','*.stt','*.res'\
                                '*.mtx','*.pes','*.par','*.pmg','*.ipm','*.pyc','*.for','*.f','*.fil','*.prt']
            for ext in extensions_to_delete:
                # glob.glob finds all files matching the pattern
                files_to_remove = glob.glob(ext)
                for f in files_to_remove:
        ...
        if k==2 and abs(incre)!=0 and stepdata[-1][-1]<0 and stepdata[-1][5]!=0:
            ...
            # CRACK PROPAGATION AND ARREST STAGE: displacement control
                ...
                # analysis files
                    ...
                    worker_script = "runrestart_job.sh"
                    ...
                    for f in export_fil:
                        shutil.move(f, work_dirkm)
                    for Dn in range(1,len(Uinterprops)+1):
                        nameinpDNk=nameinp+str(k-1)+'_N'+str(Dn)+'_K'+str(k)
                        job_name = '{}'.format(nameinpDNk)
                        work_dir = os.path.join(actual_directory, os.path.basename(job_name + '_scratch'))
                        shutil.rmtree(work_dir)
                    # delete unnecessary .inp files in the parent directory
                    list_delete = ('*.inp','*.ipm','*.pyc','*.res','*.mdl','*.stt')
                    borrar_archivos(list_delete)
                    ...
                    num = 35
                    power = float(2.25) # bias towards the initiation load
                    ...
                    for indx,value in enumerate(load_vector):
                        ...
                        while advance_iternum<numiter_max:
                            ...
                            # 1. DEFINE PATHS
                            # ---------------------------------------------------------
                            # Get the directory where your main Python script lives
                            try:
                                base_dir = actual_directory
                            except NameError:
                                base_dir = os.path.dirname(os.path.abspath(sys.argv[0]))

                            # Source: The master copy of the shell script (in Base Dir)
                            source_script = os.path.join(base_dir, "runrestart_job.sh")

                            # Destination: The local copy we will run (in Work Dir)
                            dest_script = os.path.join(work_dirkm, "runrestart_job.sh")

                            # 2. COPY AND SANITIZE (Fixes Windows \r\n and Missing File)
                            # ---------------------------------------------------------
                            if os.path.exists(source_script):
                                # Read binary to ignore encoding issues
                                with open(source_script, 'rb') as f_in:
                                    content = f_in.read()
                                
                                # Replace Windows CRLF (\r\n) with Linux LF (\n)
                                # This PREVENTS the "No such file" interpreter error
                                content = content.replace(b'\r\n', b'\n')
                                
                                # Write to the working directory
                                with open(dest_script, 'wb') as f_out:
                                    f_out.write(content)
                                    
                                print("Worker script prepared in: {}".format(dest_script))
                            else:
                                print("CRITICAL ERROR: runrestart_job.sh not found at: {}".format(source_script))
                                # You might want to exit here: sys.exit(1)

                            # 3. MAKE EXECUTABLE
                            # ---------------------------------------------------------
                            st = os.stat(dest_script)
                            os.chmod(dest_script, st.st_mode | stat.S_IEXEC)

                            # 4. EXECUTE
                            # ---------------------------------------------------------
                            print("Submitting restart job...")

                            # Use the FULL PATH to the destination script
                            command = [
                                dest_script,    # This is /full/path/to/scratch/runrestart_job.sh
                                inpfil_name,
                                oldjob_name
                            ]
                            ...
                            if len(NeL2damageKN)>0 and epsb2>bisectol2:
                                ...
                                chdir(work_dirkm)
                                if indx>=0 and advance_iternum>=3:
                                    # Remove unnecessary files for the actual iteration
                                    iter_filename = odbdef+'_m'+str(indx+1)+'_bisiter_'+str(advance_iternum-1)
                                    del_list = [iter_filename+'.stt',iter_filename+'.res',iter_filename+'.mdl',
                                                iter_filename+'.prt',iter_filename+'.msg',iter_filename+'.inp']
                            ...
                            if len(NeL2damageKN)>0  and epsb2<bisectol2:
                                ...
                                chdir(work_dirkm)
                                # List of file extensions to delete
                                extensions_to_delete = ['*.sim', '*.com', '*.jnl','*.txt','*.png'\
                                                '*.mtx','*.pes','*.par','*.pmg','*.ipm','*.pyc','*.fil']
                                for ext in extensions_to_delete:
                                    # glob.glob finds all files matching the pattern
                                    files_to_remove = glob.glob(ext)
                            ...
                            if len(NeL2damageKN)>=0 and advance_iternum==numiter_max:
                                ...
                                # List of file extensions to delete
                                extensions_to_delete = ['*.dat', '*.sim', '*.com', '*.jnl'\
                                                '*.mtx','*.pes','*.par','*.pmg','*.ipm','*.pyc','*.fil','*.odb']
                                for ext in extensions_to_delete:
                                    # glob.glob finds all files matching the pattern
                                    files_to_remove = glob.glob(ext)
                                    for f in files_to_remove:
                    ...
                    num = 35
                    power = float(2.25)  # Bias towards the start
                    ...
                    for index, val in enumerate(load_vector):
                        ...
                        # Source: The master copy of the shell script (in Base Dir)
                        source_script = os.path.join(base_dir, worker_script_bat)

                        # Destination: The local copy we will run (in Work Dir)
                        dest_script = os.path.join(work_dirkm, worker_script_bat)
                        ...
                        if os.path.exists(source_script):
                            ...
                            print("CRITICAL ERROR: run_job.sh not found at: {}".format(source_script))
                            # You might want to exit here: sys.exit(1)
                        ...
                        # Call the .sh file and pass the job_name as an argument
                        command = [
                            worker_script_bat,
                            job_name,
                    ...
                    sys.exit()
            # FORCE CONTROL CRACK PROPAGATION
            else:
                ...
                for m in range(1,msubsteps+1):
                    ...
                    print("Submitting job via Worker .sh file...")
                    # We use .call() because the .bat file is fast
                    # and will exit as soon as the job is submitted.
                    subprocess.call(command, cwd=work_dirkm)
                    ...
                    # # Remove unnecessary files for each iteration but the last one
                    # for iterindx in range(1,msubsteps):
                    #     iter_filename=odbdef+'_m'+str(iterindx)
                    #     del_list = [iter_filename+'.stt',iter_filename+'.res',iter_filename+'.mdl',
            ...
            if NeL2damagekm1!=0 and stepdata[-2][-1]>0:
                ...
                if matchdam.size > 0:
                    ...
                    matchdam0 = np.where((steps_data[:,5] == 0) & (steps_data[:,-1] >= 0))[0]
                    km1_indx = matchdam0[-1]
                    Fkm1dam0 = steps_data[km1_indx,1]
                    incre = Fk - Fkm1dam0
                    newload = Fk + incre
            elif NeL2damagekm1!=0 and stepdata[-2][-1]<0:
