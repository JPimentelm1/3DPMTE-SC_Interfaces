...
c     Rigideces del resorte dependiendo del dagno. Si esta dagnado es cero.
      ...
      IF (KINC.GT.1) THEN
        KnnE=Knn*funN*(1.0d0 - statev(1))
        KttE1=Ktt1*funT*(1.0d0 - statev(1))
        KttE2=Ktt2*funT*(1.0d0 - statev(1))
        !x Knn (in consistent units)
        K_penalty = TWO*KnnE*(1.0d0 - statev(1)) 
      END IF
      ! Nodes along the horizontal symmetry plane (e.g., Y = 0)
      IF (abs(COORDS(2)) .LT. 1.0d-7) THEN
        ! Force the normal relative displacement to be the only active one
        ! Manually zero out tangential components to prevent 'leakage'
        RDISP(3) = 0.0d0 
        STRESS(3) = 0.0d0
        ! Force the normal vector used for stress projection to be (0, 1, 0)
        ! 1. Override the Local Normal to be perfectly vertical (Global Y)
        ALOCALDIR(1, 1) = 0.0d0
        ALOCALDIR(1, 2) = 1.0d0
        ALOCALDIR(1, 3) = 0.0d0

        ! 2. Ensure Tangent 1 is perfectly aligned with Global X (or Z)
        ! This prevents shear leakage across the symmetry line
        ALOCALDIR(2, 1) = 0.0d0
        ALOCALDIR(2, 2) = 0.0d0
        ALOCALDIR(2, 3) = 1.0d0

        ! 3. Re-calculate Tangent 2 using a cross product to maintain orthonormality
        ! Or simply hard-code it if the orientation is fixed
        ALOCALDIR(3, 1) = 0.0d0
        ALOCALDIR(3, 2) = 1.0d0
        ALOCALDIR(3, 3) = 0.0d0
        IF (abs(COORDS(2)).lt.1.0d-7.AND.((COORDS(1)-0.5d0).LT.1.0d-7)) 
     &      THEN
            write(*,*) '--- Symmetry Node Debug ---'
            write(*,*) 'Node Coords:', COORDS(1), COORDS(2), COORDS(3)
            write(*,*) 'Local Normal (n)  :', ALOCALDIR(1,1), 
     &      ALOCALDIR(1,2), ALOCALDIR(1,3)
            write(*,*) 'Local Tangent 1 (t1):', ALOCALDIR(2,1), 
     &      ALOCALDIR(2,2), ALOCALDIR(2,3)
            write(*,*) 'Local Tangent 2 (t2):', ALOCALDIR(3,1), 
     &      ALOCALDIR(3,2), ALOCALDIR(3,3)
            write(*,*) '---------------------------'
        END IF
      END IF
C     *****************************************************************      
...
C        psig1=datan2(STRESS(2)*dsqrt(Knn/Ktt1),-STRESS(1))
        psig1=datan2(ABS(taun*dsqrt(Knn/Ktt1)),-STRESS(1))
...
!	          CALL SORT_FAILED_DATA(FAILED_DATA, NODNUM, 6)
     ...
     &          .and.(abs(Nini-ONE).le.1d-18).AND.(COORDS(1).GE.ZERO))
     &              THEN
                damageE=ONE
...
!     &          int(FAILED_DATA(i,2)), int(damageE), FAILED_DATA(i,3)
                ...
                ELSE IF (GtotT/tcrand.GE.(mint2tc+(ZERO*(deltat2tc)))
     &          .and.(abs(Nini-two).le.1d-18).AND.(COORDS(1).LT.ZERO))
     &          THEN
                damageE=ONE
!                    if (NODE.EQ.FAILED_DATA(i,2)) then
...
!     &          THEN
            if (GtotE.GE.(GcE).AND.
     &      (abs(Nini-ONE).le.1d-18).AND.(COORDS(1).GE.ZERO)) then
 	          damageE=ONE
 	      else if (GtotE.GE.(GcE).AND.
     &      (abs(Nini-TWO).le.1d-18).AND.(COORDS(1).LT.ZERO)) then
 	          damageE=ONE 
 	      else if (GtotE.GE.(GcE).AND.(abs(Nini-THREE).le.1d-18)) then
 	          damageE=ONE 
 	      else if (GtotE.GE.(GcE).AND.(abs(Nini-FOUR).le.1d-18)) then
 	          damageE=ZERO
...
C           damageE value for the single fibre
 	      if (GtotE.GE.(GcE).AND.
     &      (abs(Nini-ONE).le.1d-18).AND.(COORDS(1).GE.ZERO)) then
 	          damageE=ONE
 	      else if (GtotE.GE.(GcE).AND.
...
!                write(*,*) mu, GcT*mu, GtotE, statev(6)
...
!     &          ALLWKv, ALLSEv, control
                ...
                ELSE IF ((KINC.GT.15).AND.(endflag.EQ.TRUE)) THEN
...
C     ver apuntes de goodnote       
      ...
      !se hace cero para que no sume en el funcional de energia
        GcE=0.d0 
C       GtotE=GiE+GiiE+GiiiE
        GtotE=GiE+GiiE+GiiiE
      ELSE IF (damagemenosK.GT.1d-18) THEN 
...
C     _____________________________________________
      ...
      ENDIF
      IF (LOP .EQ. 1) THEN
         ! Start of the analysis increment
      ...
      IF ((LOP == 5)) THEN
        ...
        ! reset stress crit. node count to zero
        NODNUM = 0.d0
      ENDIF
      
      IF (LOP == 2) THEN ! End of the analysis increment, endflag=TRUE 
        endflag = .TRUE.
        CALL GETOUTDIR(OUTDIR, LENOUTDIR)
        program_path=OUTDIR
     ...
     &              NUM_BROKEN_INC
         IF ((KINC.GE.3) .AND. (NUM_BROKEN_INC .EQ. 0)) THEN
         WRITE(*,*) 'Bed of springs not broken (D=0): solver exit'
         CALL XIT()
         END IF
      END IF
